...
* Client API简介
    ...
    (2) 创建和配置Client实例
        ...
        Client client = ClientBuilder.newClient();
        ```
        ClientBuilder是JAX-RS的一个API，可以用于创建Client实例；它还有额外的功能，就是可以配置
        client（例如SSL传输设置）；另外一种配置client的方式是,在创建client的时候，通过传进一个
        `ClientConfig`类来进行配置；`ClientConfig`是`Configurable`的一种实现，`Configurable`
        提供了注册providers和设置properties的方法；下面代码展示如何注册providers：
        ```java
        ...
        ```
        如果想要获取当前client的配置信息，那么可以使用`getConfiguration()`方法获取：
        ```java
        ClientConfig clientConfig = new ClientConfig();
        clientConfig.register(MyClientResponseFilter.class);
        clientConfig.register(new AnotherClientFilter());
...
* Client传输连接
    ...
    默认情况下，Jersey使用HttpUrlConnection作为传输链接，它是由HttpUrlConnectorProvider实现；
    你可以使用自己的Connector或者实现Client这个接口去代替掉默认的 HttpUrlConnection;

* 使用Client 请求和响应过滤
    
    使用具有过滤的请求和响应，在底层的概念上会非常有用，这样做，你可以在应用层上面专注其他特定的领域
    而不需要考虑这些已经处理好的东西；
    Filter可以读取请求的URI，headers和entity或者读取你的响应的状态，headers和entity；
...
* 保护客户端
    ...
    客户端(client)可以使用SSL进行加密；之前上面说过SSL配置是通过Client Builder，它提供了三
